...
- [ ] 2.1 Define a `platforms` array with at least 4 hand-coded platform objects (`{x, y, w, h}`)
- [ ] 2.2 Draw all platforms in `draw()` using `rect()`
...
- [ ] 3.1 Define a `player` object with `{x, y, vx, vy, w, h, facingRight}` and place it on the top platform at startup
...
- [ ] 4.1 Define a `bullets` array; on SPACE keypress spawn a bullet `{x, y, vx}` at the player's position travelling in `player.facingRight` direction
...
- [ ] 6.1 Add `getAudioContext().resume()` on first keypress to unlock browser audio
- [ ] 6.2 Implement `playShootSound()` — short irritating high-pitched oscillator beep using `p5.Oscillator` + `p5.Env`
